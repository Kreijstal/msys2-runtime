		   ...
		   PDISPATCHER_CONTEXT dispatch)
{
  ...
  if ((NTSTATUS) e->ExceptionCode == STATUS_ACCESS_VIOLATION)
    small_printf ("ARM64_ACCESS_VIOLATION winpid=%u pc=%p sp=%p fault=%p fork=%d\r\n",
		  GetCurrentProcessId (), (void *) in->_CX_instPtr,
		  (void *) in->_CX_stackPtr,
		  (void *) e->ExceptionInformation[1], __in_forkee);
#endif
